...
extension PlanGroupViewController: UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if editingStyle == .delete{
            let plan = self.planGroup.getPlans(date: selectedDate
)[indexPath.row]
            ...
                        
                // 선택된 row의 플랜을 가져온다
                let plan = self.planGroup.getPlans(date: self.selectedDate
)[indexPath.row]
    ...
    let from = planGroup.getPlans(date: selectedDate
)[sourceIndexPath.row]
    let to = planGroup.getPlans(date: selectedDate
...
extension PlanGroupViewController: UITableViewDataSource {
    ...
    return planGroup.getPlans(date: selectedDate
).count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let plan = planGroup.getPlans(date: selectedDate
)[indexPath.row]  // 적절히 cell에 데이터를 채움
...
                planDetailViewController.plan = planGroup.getPlans(date: selectedDate
)[row].clone()
            }
        }
